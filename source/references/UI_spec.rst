...
:widgets: structure defining **input** and **output** widgets. Typically the **input** widget is set to null
  null meaning that the standard one is used. The **output** widget is frequently the name of JavaScript viewer for the output object. See the `Narrative module widgets <https://github.com/kbase/narrative/tree/develop/kbase-extension/static/kbase/js/widgets>`__ and its subdirectories for more widgets.
